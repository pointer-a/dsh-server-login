...
import { audit, createSession, createUser, deleteCredentialKey, deleteSession, findUserByUsername, listCredentialKeys, selectCredentialKey, setCredentialKey, toPublicUser, } from '../../db/repo.js';
...
export const authRoutes = async (app) => {
    ...
    app.post('/api/me/keys/:id/select', { preHandler: requireAuth }, async (request, reply) => {
        ...
        if (!selectCredentialKey(app.db, request.user.id, id))
            return reply.code(404).send({ error: 'not_found' });
        return { ok: true };
    });
    app.delete('/api/me/keys/:id', { preHandler: requireAuth }, async (request, reply) => {
        const { id } = request.params;
        if (!deleteCredentialKey(app.db, request.user.id, id))
            return reply.code(404).send({ error: 'not_found' });
        return { ok: true };
    });
};
