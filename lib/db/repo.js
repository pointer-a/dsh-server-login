...
export function setUserApiKeyRef(db, userId, encryptedRef) {
...
export function getUserApiKeyRef(db, userId) {
    const row = prepare(db, 'SELECT api_key_ref FROM users WHERE id = ?').get(userId);
