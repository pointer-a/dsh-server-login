...
export function apply(ctx) {
    ...
    console.log(`[dsh-server-login-runtime] role=${runtime.role} port=${runtime.port ?? ''} handoff=${runtime.handoffPath ?? ''} workspace=${runtime.workspace ?? ''}`);
    ...
    if (runtime.role === 'main' && runtime.workspace !== undefined && ctx.get !== undefined) {
        const registry = ctx.get('workspaceRegistry', false);
